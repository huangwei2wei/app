...
public class DomeBossBehavior extends Behavior implements Runnable, MessageCallback, MessageDispatch
{
    ...
    public DomeBossBehavior(final SpawnData data) {
        super(data);
        this.speed = new Integer(6000);
        this.reactionRadius = new Integer(70000);
        this.centerLoc = null;
    ...
    public void handleMessage(final Message msg, final int flags) {
        ...
        try {
            ...
            if (msg.getMsgType() == CombatClient.MSG_TYPE_COMBAT_LOGOUT) {
                ...
                this.threatMap.remove(playerOid);
    ...
    protected void handleDeath(final OID killer) {
        ...
        for (final OID player : this.threatMap.keySet()) {
            ...
            expMsg.setProperty("amount", (Serializable)100);
    ...
    protected void attackTarget(final OID targetOid) {
        ...
        final int attackType = random.nextInt(4);
        if (attackType == 2) {
            cooldown = 5;
            abilityID = 151;
            final ArrayList list = new ArrayList((Collection<? extends E>)this.threatMap.keySet());
        }
        CombatClient.startAbility(abilityID, this.obj.getOid(), this.currentTarget, null);
    ...
    private OID getNewTarget() {
        this.threatMap.remove(this.currentTarget);
    ...
    public class RegenerateHealth implements Runnable
    {
        @Override
        public void run() {
            if (!DomeBossBehavior.this.activated || !DomeBossBehavior.this.threatMap.isEmpty()) {
                DomeBossBehavior.this.regenerating = false;
                return;
            }
            final WorldManagerClient.ExtensionMessage regenMsg = new WorldManagerClient.ExtensionMessage(CombatClient.MSG_TYPE_REGEN_HEALTH_MANA, (String)null, DomeBossBehavior.this.obj.getOid());
    ...
    class CheckDistanceTravelled implements Runnable
    {
        ...
        private void checkDistance() {
            if (DomeBossBehavior.this.centerLoc == null) {
                return;
            }
            final InterpolatedWorldNode wnode = DomeBossBehavior.this.obj.getWorldNode();
            if (wnode == null) {
                Log.error("AGGRO: got null wnode during distance check for oid: " + DomeBossBehavior.this.obj.getOid());
                return;
            }
            final Point loc = wnode.getCurrentLoc();
            final float distance = Point.distanceTo(loc, DomeBossBehavior.this.centerLoc);
            if (distance > DomeBossBehavior.this.chaseDistance && !DomeBossBehavior.this.evade) {
                Log.debug("COMBAT: mob has exceeded max distance: " + DomeBossBehavior.this.chaseDistance);
                CombatClient.autoAttack(DomeBossBehavior.this.obj.getOid(), null, false);
                Engine.getAgent().sendBroadcast((Message)new BaseBehavior.GotoCommandMessage(DomeBossBehavior.this.obj, DomeBossBehavior.this.centerLoc, (float)DomeBossBehavior.this.speed));
                Log.debug("Evade set to true 1");
                DomeBossBehavior.this.evade = true;
                DomeBossBehavior.this.inCombat = false;
                Engine.getExecutor().schedule(this, 15000L, TimeUnit.MILLISECONDS);
                Log.debug("Evade set to true 2");
            }
            else if (DomeBossBehavior.this.evade) {
                DomeBossBehavior.this.evade = false;
