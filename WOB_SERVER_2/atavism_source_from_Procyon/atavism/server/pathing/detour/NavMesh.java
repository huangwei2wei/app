...
public class NavMesh
{
    ...
    private int FindConnectingPolys(final float vax, final float vay, final float vaz, final float vbx, final float vby, final float vbz, final MeshTile tile, final int side, final long[] con, final float[] conarea, final int maxcon) {
        if (tile == null) {
            return 0;
        }
    ...
    private void ConnectIntLinks(final MeshTile tile) {
        ...
        for (int i = 0; i < tile.Header.PolyCount; ++i) {
            final Poly poly = tile.Polys[i];
            poly.FirstLink = NavMesh.NullLink;
            if (poly.getType() != NavMeshBuilder.PolyTypeOffMeshConnection) {
                    ...
                    if (poly.Neis[j] != 0) {
                        if ((poly.Neis[j] & NavMeshBuilder.ExtLink) == 0x0) {
                            ...
                            if (idx != NavMesh.NullLink) {
                                ...
                                link3.BMax = (short)(b ? 1 : 0);
                                link2.BMin = (short)(b ? 1 : 0);
    ...
    private void BaseOffMeshLinks(final MeshTile tile) {
        ...
        for (int i = 0; i < tile.Header.OffMeshConCount; ++i) {
            final OffMeshConnection con = tile.OffMeshCons[i];
            final Poly poly = tile.Polys[con.Poly];
            final float[] ext = { con.Rad, tile.Header.WalkableClimb, con.Rad };
            final int p = 0;
            final float[] nearestPt = new float[3];
            final long refId = this.FindNearestPolyInTile(tile, con.Pos[p + 0], con.Pos[p + 1], con.Pos[p + 2], ext[0], ext[1], ext[2], nearestPt);
            if (refId > 0L) {
                if ((nearestPt[0] - con.Pos[p + 0]) * (nearestPt[0] - con.Pos[p + 0]) + (nearestPt[2] - con.Pos[p + 2]) * (nearestPt[2] - con.Pos[p + 2]) <= con.Rad * con.Rad) {
                    final int v = poly.Verts[0] * 3;
                    ...
                    if (tidx != NavMesh.NullLink) {
                        final int landPolyIdx = (int)this.DecodePolyIdTile(refId);
                        final Poly landPoly = tile.Polys[landPolyIdx];
                        final Link link2 = tile.Links[(int)tidx];
                        link2.Ref = (baseId | con.Poly);
                        link2.Edge = 255;
                        link2.Side = 255;
            ...
            for (int nv = poly.VertCount, j = 0; j < nv; ++j) {
                if ((poly.Neis[j] & NavMeshBuilder.ExtLink) != 0x0) {
                    ...
                    if (side == -1 || dir == side) {
                        ...
                        for (int nnei = this.FindConnectingPolys(tile.Verts[va + 0], tile.Verts[va + 1], tile.Verts[va + 2], tile.Verts[vb + 0], tile.Verts[vb + 1], tile.Verts[vb + 2], target, Helper.OppositeTile(dir), nei, neia, 4), k = 0; k < nnei; ++k) {
                            final long idx = this.AllocLink(tile);
                            if (idx != NavMesh.NullLink) {
                                final Link link = tile.Links[(int)idx];
                                link.Ref = nei[k];
    ...
    private int QueryPolygonsInTile(final MeshTile tile, final float qminx, final float qminy, final float qminz, final float qmaxx, final float qmaxy, final float qmaxz, final long[] polys, final int maxPolys) {
        if (tile.BVTree != null) {
            int node = 0;
    ...
    private long FindNearestPolyInTile(final MeshTile tile, final float centerx, final float centery, final float centerz, final float extentsx, final float extentsy, final float extentsz, final float[] nearestPt) {
