...
public class Crowd
{
                ...
                if (ag.State == CrowdAgentState.Walking) {
                    if (ag.TargetState != MoveRequestState.TargetNone) {
                        if (ag.TargetState != MoveRequestState.TargetVelocity) {
                            ...
                            if (ag.TargetState == MoveRequestState.TargetValid && ag.TargetReplanTime > TargetReplanDelay && ag.Corridor.PathCount() < CheckLookAhead && ag.Corridor.LastPoly() != ag.TargetRef) {
                                replan = true;
                            }
    ...
    public static int GetNeighbors(final float[] pos, final float height, final float range, final CrowdAgent skip, final CrowdNeighbour[] result, final int maxResult, final CrowdAgent[] agents, final int nagents, final ProximityGrid grid) {
        int n = 0;
