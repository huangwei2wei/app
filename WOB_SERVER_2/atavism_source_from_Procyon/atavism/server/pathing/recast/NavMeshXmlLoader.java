    ...
    public boolean generate(final NavMesh navMesh, final Element builderNode) {
        ...
        for (int i = 0; i < nodeList.getLength(); ++i) {
            if (nodeList.item(i).getNodeType() == 1) {
                if (nodeList.item(i).getNodeName().equals("Header")) {
                    ...
                    for (int j = 0; j < navVertsNodes.size(); ++j) {
                        final Element eElement = navVertsNodes.get(j);
                    ...
                    for (int j = 0; j < navDVertsNodes.size(); ++j) {
                        final Element eElement = navDVertsNodes.get(j);
                    ...
                    for (int j = 0; j < navDTrisNodes.size(); ++j) {
                        final Element eElement = navDTrisNodes.get(j);
                    ...
                    final List<Node> navOffMeshConnectionNodes = XMLHelper.getMatchingChildren(nodeList.item(i), "OffMeshConnection");
    ...
    protected OffMeshConnection processOffMeshConnection(final Node node) {
        ...
        if (vertsNode == null) {
            ...
            for (int i = 0; i < posNodes.size(); ++i) {
                final Element eElement = posNodes.get(i);
