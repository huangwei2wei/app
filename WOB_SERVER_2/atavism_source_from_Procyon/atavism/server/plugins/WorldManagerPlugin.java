...
public abstract class WorldManagerPlugin extends EnginePlugin implements MessageCallback, PerceiverCallback<WMWorldNode>, PerceptionUpdateTrigger
{
    ...
    protected WorldManagerClient.NewRemoteObjectMessage makeNewObjectForFixedPerceiverMessage(final FixedPerceiver<WMWorldNode> fixedP, final WMWorldNode newWnode) {
        ...
        final WorldManagerClient.NewRemoteObjectMessage newObjMsg = new WorldManagerClient.NewRemoteObjectMessage(remoteSessionId, newWnode.getInstanceOid(), newOid, newWnode.getLoc(), newWnode.getOrientation(), newWnode.getPerceptionRadius(), newObj.getType());
        return newObjMsg;
    }
    ...
    class MobPathReqHook implements Hook
    {
        ...
        public boolean processMessage(final Message msg, final int flags) {
            ...
            if (nomove) {
                final WorldManagerClient.MobPathCorrectionMessage correction = new WorldManagerClient.MobPathCorrectionMessage(oid, System.currentTimeMillis(), "linear", 0.0f, "", new LinkedList<Point>());
                ...
                if (speed != 0.0f) {
                    final PathInterpolator pathInterpolator = interpKind.equalsIgnoreCase("spline") ? new PathSpline(oid, startTime, speed, terrainString, pathPoints) : new PathLinear(oid, startTime, speed, terrainString, pathPoints);
    ...
    class PlayerPathWMReqHook implements Hook
    {
        ...
        public boolean processMessage(final Message msg, final int flags) {
            ...
            if (Log.loggingDebug) {
                WorldManagerPlugin.log.debug("PlayerPathReqWMHook.processMessage: Received a PLAYER_PATH_REQ message for player " + playerOid + " and roomId " + roomId + " with boundary " + boundary + " and obstacles " + obstacles);
            }
            if (boundary == null != (obstacles == null)) {
                WorldManagerPlugin.log.error("PlayerPathReqWMHook.processMessage: For player " + playerOid + ", received a PLAYER_PATH_REQ message for roomId " + roomId + ", but boundary is " + boundary + " but obstacles is " + obstacles);
                return false;
            }
            if (pathObject == null && boundary == null) {
                WorldManagerPlugin.log.error("PlayerPathReqWMHook.processMessage: For player " + playerOid + ", received a PLAYER_PATH_REQ message for roomId " + roomId + ", but didn't find roomId in the cache and no no boundary or obstacles were supplied.");
                return false;
            }
            if (boundary != null) {
            ...
            if (value == null) {
                WorldManagerPlugin.log.error("PlayerPathReqWMHook.processMessage: For player " + playerOid + ", roomId " + roomId + ", start " + reqMsg.getStart() + ", dest " + reqMsg.getDest() + ", cound not generate path!");
            }
            else {
                ...
                final WorldManagerClient.MobPathMessage mobPathMsg = new WorldManagerClient.MobPathMessage(playerOid, System.currentTimeMillis(), "spline", reqMsg.getSpeed(), value.getTerrainString(), pathPoints);
