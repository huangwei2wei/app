...
public class PathState {
	...
	public WorldManagerClient.MobPathReqMessage setupPathInterpolator(final long timeNow, final Point newStartLoc, final Point newEndLoc, final float newSpeed, final boolean following,
			final float followDistance, final boolean followsTerrain) {
		...
		if (result == PathSearcher.PathResult.OK && this.path.size() >= 2) {
			...
			final WorldManagerClient.MobPathReqMessage reqMsg = new WorldManagerClient.MobPathReqMessage(this.oid, this.startTime, this.linear ? "linear" : "spline", this.speed, terrainString,
					this.path);
