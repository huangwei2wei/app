...
public class PathAStarSearcher {
	...
	protected SearchState iterate() {
		...
		for (final PathSearchNode successor : successors) {
			...
			if (openElement != null) {
				if (PathAStarSearcher.logAll) {
					PathAStarSearcher.log.debug("iterate: Successor index = " + index + " found in openStates, so replacing openElement cost = " + openElement.getCostSoFar() + " with current cost = "
							+ cost);
				}
				...
				if (PathAStarSearcher.logAll) {
					PathAStarSearcher.log.debug("iterate: About to add successor current = " + current.getPolyIndex() + "; openStates.size() = " + this.openStates.size()
							+ "; openPrioritySet.size() = " + this.openPrioritySet.size());
				}
				...
				PathAStarSearcher.log.debug("iterate: Added successor current = " + current.getPolyIndex() + "; openStates.size() = " + this.openStates.size() + "; openPrioritySet.size() = "
						+ this.openPrioritySet.size());
			}
		}
	...
	public enum SearchState {
		Running((byte) 0), Succeeded((byte) 1), Failed((byte) 2);
