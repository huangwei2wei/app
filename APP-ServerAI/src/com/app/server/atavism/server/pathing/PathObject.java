...
public class PathObject implements Serializable, Cloneable {
	...
	public PathObject(final String modelName, final String type, final int firstTerrainIndex, final PathPolygon boundingPolygon, final List<PathPolygon> polygons, final List<PathArc> portals,
			final List<PathArc> arcs) {
		this.polygonArcs = null;
		this.polygonMap = null;
		this.terrainPolygonAtCorner = null;
	...
	public PathObject(final String description, final float avatarWidth, final List<AOVector> boundaryCorners, final List<List<AOVector>> obstacleCorners) {
		...
		final List<PathPolygon> triangles = triangulator.computeTriangulation(description, boundary, obstacles);
					...
					for (int polySize = polyCorners.size(), pc = 0; pc < polySize; ++pc) {
						...
						for (final PathPolygon triangle : triangles) {
							...
							for (int tc = 0; tc < 3; ++tc) {
								...
								if (((AOVector.distanceToSquared(pcCorner1, triCorner1) < 1.0f && AOVector.distanceToSquared(pcCorner2, triCorner2) < 1.0f) || (AOVector.distanceToSquared(pcCorner2,
										triCorner1) < 1.0f && AOVector.distanceToSquared(pcCorner1, triCorner2) < 1.0f)) && AOVector.counterClockwisePoints(pcCorner1, triCorner3, pcCorner2)) {
									foundOne = true;
			...
			for (int size1 = corners1.size(), pc1 = 0; pc1 < size1; ++pc1) {
				...
				for (int p2 = p1 + 1; p2 < s; ++p2) {
					...
					for (int size2 = corners2.size(), pc2 = 0; pc2 < size2; ++pc2) {
						...
						if ((AOVector.distanceToSquared(corner11, corner13) < 1.0f && AOVector.distanceToSquared(corner12, corner14) < 1.0f)
								|| (AOVector.distanceToSquared(corner12, corner13) < 1.0f && AOVector.distanceToSquared(corner11, corner14) < 1.0f)) {
