...
public class PathSpline extends PathInterpolator {
	...
	public PathSpline(final OID oid, final long startTime, final float speed, final String terrainString, final List<Point> path) {
		...
		log.debug("PathSpline constructor: oid = " + oid + "; timeVector = " + this.timeVector + "; timeVector.length = " + this.timeVector.length + "; path = " + path + "; speed = " + speed);
	...
	public PathLocAndDir interpolate(float t) {
		...
		if (pointNumber == -1) {
			log.error("interpolateSpline: Time t " + t + " passed to interpolateSpline < 0; oid = " + this.oid);
			pointNumber = 1;
		}
		AOVector loc;
		AOVector dir;
		...
		if (PathSpline.logAll) {
			log.debug("interpolateSpline: oid = " + this.oid + "; t = " + t + "; loc = " + loc + "; dir = " + dir);
		}
	...
	protected float basisFactor(final int degree, final float t) {
		switch (degree) {
			...
			default : {
				log.error("interpolateSpline: Invalid basis index " + degree + " specified! - oid = " + this.oid);
				return 0.0f;
			}
		}
	}
