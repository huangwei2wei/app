	...
	protected void generateConvexPolygons(final PathPolygon poly) {
		...
		while (origCornerNumber < size) {
			if (concaveCorners.contains(origCornerNumber)) {
				...
				for (firstNewPolyCorner = wrap(lastNewPolyCorner - 2, size); AOVector.counterClockwisePoints(points[wrap(lastNewPolyCorner - 1, size)], points[lastNewPolyCorner],
						points[firstNewPolyCorner]); firstNewPolyCorner = wrap(firstNewPolyCorner - 1, size), ++newPolyCornerCount) {
				}
				...
				PathSynth.log.debug("PathSynth.generateConvexPolygons: new poly corner count " + newPolyCorners.size() + ", first corner " + firstNewPolyCorner + ", last corner " + lastNewPolyCorner
						+ ",  " + newPoly);
