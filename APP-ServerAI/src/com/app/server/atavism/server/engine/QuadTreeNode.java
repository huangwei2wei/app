...
public class QuadTreeNode<ElementType extends QuadTreeElement<ElementType>> {
	...
	public boolean containsPointWithHysteresis(final Point loc) {
		...
		try {
			...
			final Geometry hystericalGeometry = new Geometry(this.geometry.getMinX() + hysteresis, this.geometry.getMaxX() - hysteresis, this.geometry.getMinZ() + hysteresis, this.geometry.getMaxZ()
					- hysteresis);
	...
	QuadTreeNode<ElementType> whichChild(final Point loc) {
		...
		try {
			...
			log.warn("whichChild: did not find child for point " + loc + ", thisNode=" + this);
			return null;
		} finally {
			this.lock.unlock();
		}
		...
		try {
			if (!this.isLeaf()) {
				...
				for (final QuadTreeNode<ElementType> child : this.children) {
					...
					log.debug("getElementsBetween: child = " + child + "; loc1 = " + loc1 + "; loc2 = " + loc2);
			...
			log.debug("getElementsBetween leaf: geometry = " + this.geometry + "; nodeElements.size() = " + this.nodeElements.size() + "; loc1 = " + loc1 + "; loc2 = " + loc2);
		...
		try {
			if (this.isLeaf()) {
				if (this.distanceTo(loc) < radius) {
					...
					log.debug("addPerceiverExtentObject; adding: " + this.toString() + " elem: " + elem + ", loc: " + loc + " radius: " + radius);
	...
	public void removePerceiverExtentObject(final ElementType elem) {
		...
		try {
			if (this.isLeaf()) {
				...
				log.debug("removePerceiverExtentObject; removing: " + this.toString() + " elem: " + elem);
	...
	static boolean segmentCloserThanDistance(final Point loc1, final Point loc2, final float centerX, final float centerZ, final float radius) {
		...
		log.debug("segmentCloserThanDistance: centerX = " + centerX + "; centerZ = " + centerZ + "; radius = " + radius);
		...
		try {
			...
			try {
				...
				for (final ElementType elem : this.nodeElements) {
					...
					if (childNode == null) {
						QuadTreeNode.log
								.debug("divide: world node is no longer in this quad tree node, skipping it.  it should be moved when the updater thread notices its not longer in the quad tree node anymore");
					} else {
