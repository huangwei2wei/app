...
public class NavmeshLine2D {
	 ...
	 * @return
	 */
	// public static boolean checkLineIn(List<NavmeshLine2D> allLines, NavmeshLine2D chkLine, int index)
	// {
	// index = -1;
	...
	public NavmeshPointSide classifyPoint(Vector2 point) {
		...
		Vector2 vectorA = Vector2.sub(_endPoint, _startPoint);// _endPoint - _startPoint;
	...
	public Vector2 getDirection() {
		Vector2 dir = Vector2.sub(_endPoint, _startPoint);// _endPoint - _startPoint;
		return dir;
	}
	...
	public float getLength() {
		return (float) Math.sqrt(Math.pow(_startPoint.x - _endPoint.x, 2.0) + Math.pow(this._startPoint.y - _endPoint.y, 2.0));
	}
	...
	public boolean equals(NavmeshLine2D line) {
		if (NavmeshMath.isEqualZero(Vector2.sub(line._startPoint, line._endPoint)) || NavmeshMath.isEqualZero(Vector2.sub(_startPoint, _endPoint)))
			return false;
		boolean bEquals = NavmeshMath.isEqualZero(Vector2.sub(_startPoint, line._startPoint)) ? true : NavmeshMath.isEqualZero(Vector2.sub(_startPoint, line._endPoint));
		if (bEquals)
			bEquals = NavmeshMath.isEqualZero(Vector2.sub(_endPoint, line._startPoint)) ? true : NavmeshMath.isEqualZero(Vector2.sub(_endPoint, line._endPoint));
		return bEquals;
	}
