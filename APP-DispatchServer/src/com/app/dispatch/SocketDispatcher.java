...
public class SocketDispatcher implements Dispatcher, Runnable {
	...
	// private SyncService syncService = null;// 同城地图同步
	// private PvpService pvpService = null;// pvp 同屏同步
	...
	public void dispatchToServer(IoSession session, Object object) {
		...
		if (type == Protocol.MAIN_ACCOUNT) {
			if (subType == Protocol.ACCOUNT_Heartbeat && (Boolean) session.getAttribute(LOGINMARK_KEY)) {// 回应客户端心跳协议
				...
				buffer.putInt(4, sessionId.intValue());// sessionId
				this.serverSession.write(buffer.duplicate());// 发送worldServer
			}
			// } else if (type == Protocol.MAIN_SYN) {// 同步协议
			// ClientInfo client = (ClientInfo) session.getAttribute(CLIENTINFO_KEY);
			// Player player = client.getPlayer();
			// try {
			// if (subType == Protocol.SYN_Move) {// 玩家移动协议处理
			// INetData data = new S2SData(Arrays.copyOfRange(buffer.array(), 18, buffer.array().length), 1, -1);
			// int roomId = player.getRoomId();
			// int mapId = player.getMapId();
			// if (roomId > 0)// 在pvp 内
			// this.pvpService.broadcastingMove(client, data, this.channelService);
			// else if (mapId > 0)// 在世界地图内
			// this.syncService.broadcastingMove(client, data);
			// } else if (subType == Protocol.SYN_ReportPlace) {// 在世界地图内客户端告知服务器位置
			// INetData data = new S2SData(Arrays.copyOfRange(buffer.array(), 18, buffer.array().length), 1, -1);
			// this.syncService.reportPlace(client, data);
			// } else if (subType == Protocol.SYN_Attack) {// pvp 内攻击
			// INetData data = new S2SData(Arrays.copyOfRange(buffer.array(), 18, buffer.array().length), 1, -1);
			// this.pvpService.broadcastingAttack(client, data, this.channelService, this.allClientInfo);// 广播技能，攻击等
			// }
	...
	public boolean checkProtocol(IoSession session, byte type, byte subType) {
		...
		} else if (type == Protocol.MAIN_SYN) {// 同屏协议
			if (subType == Protocol.SYN_Move) {// 移动协议
