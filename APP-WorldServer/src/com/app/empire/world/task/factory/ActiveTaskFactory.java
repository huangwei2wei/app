 ...
 *
 */
public class ActiveTaskFactory extends AbstractTaskTriggerFactory {

	private static volatile ActiveTaskFactory instance;
	
	public static ActiveTaskFactory getInstance(){
		if(instance==null){
			synchronized (ActiveTaskFactory.class) {
				if(instance == null){
					instance = new ActiveTaskFactory();
				}
			}
		}
		return instance;
...
//				break;	
		...
		if(trigger == null){
			Log.error("ActiveTaskFactory工厂里没有此类型监听器："+cfg.getTaskTarget());
		}
		return trigger;
	}
...
//				break;	
		...
		if(trigger == null){
			Log.error("ActiveTaskFactory工厂里没有此类型初始化形为处理器："+cfg.getTaskTarget());
		}
		return trigger;
	}
