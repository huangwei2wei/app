...
public class NavmeshTriangle {
	...
	public NavmeshTriangle(int id, Vector3 pos1, Vector3 pos2, Vector3 pos3) {
		...
		_points3D[2] = pos3;
		_points = new Vector2[3];
		_points[0] = new Vector2(pos1.x, pos1.z);
		_points[1] = new Vector2(pos2.x, pos2.z);
		_points[2] = new Vector2(pos3.x, pos3.z);
		_wallDistance = new double[3];
		_neighdorIDs = new int[] { -1, -1, -1 };
		// 计算中心点
		...
		_wallDistance[0] = Math.sqrt((wallMidPoint[0].x - wallMidPoint[1].x) * (wallMidPoint[0].x - wallMidPoint[1].x)
				+ (wallMidPoint[0].y - wallMidPoint[1].y) * (wallMidPoint[0].y - wallMidPoint[1].y));
		_wallDistance[1] = Math.sqrt((wallMidPoint[1].x - wallMidPoint[2].x) * (wallMidPoint[1].x - wallMidPoint[2].x)
				+ (wallMidPoint[1].y - wallMidPoint[2].y) * (wallMidPoint[1].y - wallMidPoint[2].y));
		_wallDistance[2] = Math.sqrt((wallMidPoint[2].x - wallMidPoint[0].x) * (wallMidPoint[2].x - wallMidPoint[0].x)
	...
	public boolean isPointIn(Vector2 pt) {
		...
		if (resultA == NavmeshPointSide.ON_LINE || resultB == NavmeshPointSide.ON_LINE
				|| resultC == NavmeshPointSide.LEFT_SIDE)
			return true;
		else if (resultA == NavmeshPointSide.RIGHT_SIDE && resultB == NavmeshPointSide.RIGHT_SIDE
				&& resultC == NavmeshPointSide.RIGHT_SIDE)
			return true;
		return false;
	}
	...
	public double getWallDis(int index) {
		if (index >= 3) {
			...
			System.err.println("setWallDis:The index is large than 3.");
			return -1;
		}
