...
public class MessageAgent implements MessageIO.Callback, TcpAcceptCallback, ResponseCallback {
	...
	public void sendResponse(final ResponseMessage message) {
		message.setMessageId(this.nextMessageId());
		if (Log.loggingDebug) {
			Log.debug("sendResponse to " + message.getRequestingAgent().agentName + "," + message.getRequestId() + " type=" + message.getMsgType().getMsgTypeString() + " id=" + message.getMsgId()
	...
	public void handleMessageData(final int length, final AOByteBuffer messageData, final AgentInfo agentInfo) {
		if (length == -1 || messageData == null) {
			...
			if (Log.loggingDebug) {
				...
				Log.debug("handleMessageData from " + agentInfo.agentName + "," + message.getMsgId() + responseTo + " type=" + msgType.getMsgTypeString() + " len=" + length + " class="
	...
	private void deliverMessage(final Message message) {
		...
		if (message.isRPC()) {
			try {
				...
				if (rpcCount == 0) {
					final ExceptionResponseMessage response = new ExceptionResponseMessage(message, new NoRecipientsException("sendRPC: no message recipients for type=" + message.getMsgType() + " "
		...
		public void run() {
			try {
				...
				if (remoteAgent == null) {
					...
					final AgentStateMessage agentState = new AgentStateMessage(MessageAgent.this.agentId, MessageAgent.this.agentName, ":same", MessageAgent.this.agentPort,
							MessageAgent.this.domainFlags);
			...
			while (true) {
				try {
					while (true) {
						...
						Log.info("MessageAgent Local Subscription Counters: last interval/total: Created "
								+ (MessageAgent.localSubscriptionCreatedCount - MessageAgent.lastLocalSubscriptionCreatedCount) + "/" + MessageAgent.localSubscriptionCreatedCount + ", Removed "
								+ (MessageAgent.localSubscriptionRemovedCount - MessageAgent.lastLocalSubscriptionRemovedCount) + "/" + MessageAgent.localSubscriptionRemovedCount);
						Log.info("MessageAgent Remote Subscription Counters: last interval/total: Created "
								+ (MessageAgent.remoteSubscriptionCreatedCount - MessageAgent.lastRemoteSubscriptionCreatedCount) + "/" + MessageAgent.remoteSubscriptionCreatedCount + ", Removed "
								+ (MessageAgent.remoteSubscriptionRemovedCount - MessageAgent.lastRemoteSubscriptionRemovedCount) + "/" + MessageAgent.remoteSubscriptionRemovedCount);
						Log.info("MessageAgent Filter Updates: last interval/total: Local " + (MessageAgent.localFilterUpdateCount - MessageAgent.lastLocalFilterUpdateCount) + "/"
								+ MessageAgent.localFilterUpdateCount + ", Remote " + (MessageAgent.remoteFilterUpdateCount - MessageAgent.lastRemoteFilterUpdateCount) + "/"
								+ MessageAgent.remoteFilterUpdateCount);
