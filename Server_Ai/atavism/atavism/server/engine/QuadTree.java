...
public class QuadTree<ElementType extends QuadTreeElement<ElementType>>
{
    ...
    public void addFixedPerceiver(final FixedPerceiver<ElementType> perceiver) {
        if (Log.loggingDebug) {
            QuadTree.log.debug("QuadTree.addFixedPerceiver p=" + perceiver);
        }
    ...
    protected QuadTreeNode<ElementType> addHelper(final QuadTreeNode<ElementType> node, final ElementType elem, final Point loc, final NewsAndFrees newsAndFrees) {
        ...
        if (Log.loggingDebug) {
            QuadTree.log.debug("QuadTree.addHelper: node is leaf: " + node + ", rootNode is leaf? " + this.rootNode.isLeaf() + ", qtree=" + this.hashCode());
        }
        final int curSize = node.numElements();
        final int maxSize = this.getMaxObjects();
        if (curSize >= maxSize && node.getDepth() < this.getMaxDepth()) {
            if (Log.loggingDebug) {
                QuadTree.log.debug("QuadTree.addHelper: maxObj=" + maxSize + ", cursize=" + curSize + ".. dividing");
            }
            node.divide(newsAndFrees);
            return this.addHelper(node, elem, loc, newsAndFrees);
        }
        if (Log.loggingDebug) {
            QuadTree.log.debug("QuadTree.addHelper: adding element " + elem + " to quadnode " + node + " -- maxObjects=" + this.getMaxObjects());
        }
        this.updateElementInternal(elem, loc, newsAndFrees);
    ...
    protected void updateElementInternal(final ElementType elem, final Point loc, final NewsAndFrees newsAndFrees) {
        ...
        try {
            if (Log.loggingDebug) {
                QuadTree.log.debug("updateElement: elem=" + elem);
            }
            final QuadTreeNode<ElementType> node = elem.getQuadNode();
            if (node != null && node.getGeometry().contains(loc)) {
                ...
                if (perceiver != null) {
                    ...
                    QuadTree.log.debug("updateElementInternal: has no perceiver");
                }
                return;
            }
            final QuadTreeNode<ElementType> newNode = this.findLeafNode(loc);
            if (node != null) {
                ...
                if (Log.loggingDebug) {
                    QuadTree.log.debug("QuadTree.updateElementInternal: element moved out of node obj=" + elem.getQuadTreeObject() + " oldNode=" + node + " newNode=" + newNode + ", loc=" + loc);
                }
            }
    ...
    protected void updateElementPerceiversInternal(final ElementType elem, final QuadTreeNode<ElementType> oldNode, final QuadTreeNode<ElementType> newNode, final NewsAndFrees newsAndFrees) {
        ...
        if (Log.loggingDebug) {
            QuadTree.log.debug("updateElementPerceivers: elem=" + elem + " oldNode=" + oldNode + " newNode=" + newNode);
        }
        if (oldNode != null) {
        ...
        if (Log.loggingDebug) {
            QuadTree.log.debug("updateElementPerceivers: remove perceivers size=" + removePerceivers.size() + "add perceivers size=" + addPerceivers.size());
        }
    ...
    protected void updatePerceiverInternal(final Perceiver<ElementType> perceiver, final NewsAndFrees newsAndFrees) {
        ...
        try {
            if (perceiver instanceof MobilePerceiver) {
                ...
                if (Log.loggingDebug) {
                    QuadTree.log.debug("QuadTree.updatePerceiver: mobile perceiver radius=" + p.getRadius() + " owner=" + p.getElement().getQuadTreeObject());
                }
            }
            else {
                final FixedPerceiver<ElementType> p2 = (FixedPerceiver<ElementType>)(FixedPerceiver)perceiver;
                if (Log.loggingDebug) {
                    QuadTree.log.debug("QuadTree.updatePerceiver: fixed perceiver geom=" + p2.getGeometry());
                }
            }
            ...
            if (Log.loggingDebug) {
                QuadTree.log.debug("Before removing, newPerceiverExtentElements.size(): " + newPerceiverExtentElements.size() + " oldPerceiverExtentElements.size(): " + oldPerceiverExtentElements.size() + " bothPerceiverExtentElements.size(): " + bothPerceiverExtentElements.size() + " num remove nodes=" + removeNodes.size());
            }
            for (final QuadTreeNode<ElementType> node2 : removeNodes) {
                if (Log.loggingDebug) {
                    QuadTree.log.debug("updatePerceiver: removing from node " + node2);
                }
            ...
            if (Log.loggingDebug) {
                QuadTree.log.debug("After removing, newPerceiverExtentElements.size(): " + newPerceiverExtentElements.size() + " oldPerceiverExtentElements.size(): " + oldPerceiverExtentElements.size());
            }
            for (final ElementType elem3 : oldPerceiverExtentElements) {
                newsAndFrees.noteFreedElement(perceiver, elem3);
            }
            if (Log.loggingDebug) {
                QuadTree.log.debug("updatePerceiver: num addnodes=" + addNodes.size());
            }
            for (final QuadTreeNode<ElementType> node2 : addNodes) {
                if (Log.loggingDebug) {
                    QuadTree.log.debug("updatePerceiver: adding to node " + node2);
                }
            ...
            QuadTree.log.debug("updatedPerceiver: done updating, printing out list of all nodes");
            if (Log.loggingDebug) {
                for (final QuadTreeNode<ElementType> node2 : perceiver.getQuadTreeNodes()) {
                    QuadTree.log.debug("updatePerceiver: IS IN NODE " + node2);
                }
            }
        }
        finally {
    ...
    protected void setLocalGeometryHelper(final QuadTreeNode<ElementType> node, final Geometry g, final NewsAndFrees newsAndFrees) {
        ...
        if (g.contains(node.getGeometry())) {
            ...
            if (node.isLeaf()) {
                QuadTree.log.debug("setLocalGeometryHelper: divide");
                node.divide(newsAndFrees);
            }
    ...
    public class NewsAndFrees
    {
        ...
        protected Integer processBatchedNewsAndFrees(final OID mobilePerceiverOid) {
            ...
            if (Log.loggingDebug && (frees > 0 || news > 0)) {
