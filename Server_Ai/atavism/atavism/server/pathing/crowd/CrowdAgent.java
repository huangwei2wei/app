...
public class CrowdAgent {
	...
	public int NNeis;
	public float DesiredSpeed;// 角色速度
	public float[] npos;// 现在的位置
	public float[] disp;
	public float[] dvel;
	public float[] nvel;
	public float[] vel;// xyz 方向平均速度
	public CrowdAgentParams Param;
	public float[] CornerVerts;
	public short[] CornerFlags;
	public long[] CornerPolys;
	public int NCorners;
	public MoveRequestState TargetState;// 目标状态
	public long TargetRef;// 是否有要去的地方0 没有
	public float[] TargetPos;// 要去的地方
	public long TargetPathQRef;// 目标路径
	public Boolean TargetReplan;// 目标计划
	...
	public Boolean OverOffMeshConnection(final float radius) {
		...
		if (offMeshConnection) {
			final float distSq = Helper.VDist2DSqr(this.npos[0], this.npos[1], this.npos[2], this.CornerVerts[(this.NCorners - 1) * 3 + 0], this.CornerVerts[(this.NCorners - 1) * 3 + 1],
					this.CornerVerts[(this.NCorners - 1) * 3 + 2]);
