...
public class PhysicalDotEffect extends AgisEffect
{
    ...
    static {
        PhysicalDotEffect.random = new Random();
    }
    
    public PhysicalDotEffect(final int id, final String name) {
        super(id, name);
        this.minDmg = 0;
        this.maxDmg = 0;
        this.pulseDamage = 0;
    ...
    public void apply(final EffectState state) {
        ...
        if (this.maxDmg > this.minDmg) {
            dmg += PhysicalDotEffect.random.nextInt(this.maxDmg - this.minDmg);
        }
        dmg = CombatHelper.CalcMeleeDamage(target, source, dmg, this.damageType, this.skillEffectMod.get(0), this.effectSkillType, this.hitRoll, false);
